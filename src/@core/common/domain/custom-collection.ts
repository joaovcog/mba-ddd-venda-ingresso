...
export class CustomCollectionFactory {
  static create<T extends object>(ref: object): ICollection<T> {
    const collection = new Collection<T>(ref);
    collection['initialized'] = false;
    return CustomCollectionFactory.createProxy(collection);
  }

  static createFrom<T extends object>(target: Collection<T>): ICollection<T> {
    return CustomCollectionFactory.createProxy(target);
  }
    ...
    return new Proxy(target, {
      get(target, prop, receiver) {
        ...
        return Reflect.get(
          ...
          prop,
          receiver,
        ) as unknown as ICollection<T>[keyof ICollection<T>];
      },
    });
  }
}
